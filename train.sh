...
usage() {
    echo "Usage:$0 mode task [train_steps] [update_freq] [max_tokens_per_batch]"
    exit 1
}
if [ $# -lt 2 ];then
    usage;
...
if [ ! -z $_update_freq ] && [ ! -z $_max_tokens_per_batch ]; then
	...
	5)
	    ...
	    ;;
	6)
	    max_tokens_per_batch=$((max_tokens_per_batch*4/6))
	    ;;
	7)
	    update_freq=$((update_freq/2))
...
echo $update_freq $max_tokens_per_batch
# Memory-intensive special cases. Reduce the batch size and increase the number of gradient accumlation.
if [[ $mode =~ .ls20 ]];then
    update_freq=$((update_freq*2))
    max_tokens_per_batch=$((max_tokens_per_batch/2))
fi
if [[ $mode =~ .ls40 ]];then
    update_freq=$((update_freq*4))
    max_tokens_per_batch=$((max_tokens_per_batch/4))
fi
if [[ $mode =~ .ls80 ]];then
    update_freq=$((update_freq*8))
    max_tokens_per_batch=$((max_tokens_per_batch/8))
fi
...
if [[ ${mode} =~ .no_enc_attn\. ]]; then
    train_options="$train_options --no-encoder-attn"
fi

if [[ ${mode} =~ .maxkl([0-9.]+)\. ]]; then
    max_kl_weight=${BASH_REMATCH[1]}
    train_options="$train_options --max-kl-weight $max_kl_weight"
fi
