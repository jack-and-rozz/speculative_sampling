    ...
    def generate(
        ...
        **kwargs
        ...
        try:
            ...
            print("Error: This generator cannot handle ensembling. Load only one model.", file=sys.stderr)
            exit(1)
        ...
        if 'latent_out' in encoder_outs[0]:
            for sent in range(len(finalized)):
                for i in range(len(finalized[sent])):
                    finalized[sent][i]['latent_out'] = encoder_outs[0]['latent_out'][i]
        return finalized
